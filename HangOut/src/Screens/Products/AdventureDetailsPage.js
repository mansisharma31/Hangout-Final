...
export const AdventureDetailsPage = () => {
    const { id } = useParams();
    let adventureData = adventuresArray.find((data) => data.id == id);
    console.log(adventureData)
    if(!adventureData){
        adventureData = adventuresArray.find((data)=> data.title == id);
    }
        ...
        <div className="bg-gradient-to-b from-[#FAF7F0] to-[#D8D2C2] min-h-screen mt-12">
            ...
            <div className="container mx-auto px-6 lg:px-12 py-12">
                ...
                <p className="text-md text-[#4A4947] mb-4">
                    🕒 {adventureData.timings} | {adventureData.price}
                </p>
                ...
                <div className="grid grid-cols-3 gap-6 mb-6">
                    ...
                    <div className="flex flex-col space-y-4">
                        {adventureData.images.map((img, index) => (
                            <div
                                key={index}
            ...
            <div className="container mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-3 gap-12 py-12">
                ...
                <aside className="hidden lg:block lg:sticky lg:top-20 bg-white p-6 rounded-lg shadow-xl h-fit">
                    ...
                    <p className="text-sm text-black">{adventureData.price}</p>
                    <p className="text-sm text-gray-500 mt-4">Timings</p>
                    <p className="text-sm text-black">{adventureData.timings}</p>
                    <p className="text-sm text-gray-500 mt-4">Location</p>
                    <p className="text-sm text-black">{adventureData.locations}</p>
                    <p className="text-sm text-gray-500 mt-4">📞 {adventureData.contact}</p>
                    <a href={adventureData.website} className="text-[#B17457] mt-4 block hover:underline">
                        🌐 Visit Website
                    </a>
                </aside>
                    ...
                    <div>
                        {activeTab === "details" && (
                            <div className="bg-white p-6 rounded-lg shadow-xl mb-6">
                                ...
                                <p className="text-[#4A4947]">{adventureData.description}</p>
                            </div>
                        )}
                        {activeTab === "reviews" && (
                        ...
                        {activeTab === "location" && (
                            <div className="bg-white p-6 rounded-lg shadow-lg mb-6">
                                ...
                                <a href={adventureData.mapLink} className="text-blue-600 mt-2 block hover:underline break-words" style={{ wordBreak: "break-word", overflowWrap: "break-word" }}>{adventureData.mapLink}</a>
                            </div>
                        )}
                    </div>
                </div>
