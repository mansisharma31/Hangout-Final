...
export const TripDetailsPage = () => {
    const { id } = useParams();
    let tripData = tripsArray.find((data) => data.id == id);
    console.log(tripData)
    if(!tripData){
        tripData = tripsArray.find((data)=> data.title == id);
    }
        ...
        <div className="bg-gradient-to-b from-[#FAF7F0] to-[#D8D2C2] min-h-screen mt-12">
            ...
            <div className="container mx-auto px-6 lg:px-12 py-12">
                ...
                <p className="text-md text-[#4A4947] mb-4">
                     {tripData.price} | 🕒 {tripData.timings}
                </p>
                ...
                <div className="grid grid-cols-3 gap-6 mb-6">
                    <div className="col-span-2 relative overflow-hidden rounded-lg shadow-lg transition-transform transform hover:scale-105">
                        ...
                        <div className="absolute bottom-4 left-4 text-white">
                            <h2 className="text-xl font-bold">Experience the Best Tours and Attractions</h2>
                        </div>
                    </div>
            ...
            <div className="container mx-auto px-6 lg:px-12 grid grid-cols-1 lg:grid-cols-3 gap-12 py-12">
                ...
                <aside className="hidden lg:block lg:sticky lg:top-20 bg-white p-6 rounded-lg shadow-xl h-fit">
                    ...
                    <p className="text-sm text-black">{tripData.timings}</p>
                    <p className="text-sm text-gray-500 mt-4">Location(s)</p>
                    <p className="text-sm text-black">{tripData.departure}</p>
                    ...
                    <div>
                        {activeTab === "details" && (
                            <div className="bg-white p-6 rounded-lg shadow-xl mb-6">
                                ...
                                <p className="text-[#4A4947]">{tripData.description}</p>
                            </div>
                        )}
                        {activeTab === "reviews" && (
                        ...
                        {activeTab === "location" && (
                            <div className="bg-white p-6 rounded-lg shadow-lg mb-6">
                                ...
                                <a href={tripData.mapLink} className="text-blue-600 mt-2 block hover:underline break-words" style={{ wordBreak: "break-word", overflowWrap: "break-word" }}>{tripData.mapLink}</a>
                            </div>
                        )}
                    </div>
                </div>
